$(function(){
      ...
      success:function(data){
        ...
        if(basic!==undefined){
          for(var i=0;i<basic.length;i++){
            var location=basic[i].location;
            var parent_city=basic[i].parent_city;
            var admin_area=basic[i].admin_area;
            var cnty=basic[i].cnty;
            $("#showCityUl").append(`
            <li>${location}-${parent_city}-${admin_area}-${cnty}</li>
            `)
          }
        }
      }
  ...
  //模糊搜索列表
  $("#showCityUl").on("click","li",function(event){
    event.stopPropagation();
    var self=$(this);
    console.log(self.text());
    var city=self.text().split("-")[0];
    console.log(city);
    searchCity(city);
  })
